...
/*	
	...
	*/
    ...
    public String doLogins(@ModelAttribute("orders")  ArrayList<Item> orders,BindingResult result,@RequestParam String dates,@RequestParam String times,@RequestParam String total,@RequestParam int totaltime, Model model,HttpSession session,HttpServletRequest request) {
		...
		// Initialize Pickup , ready and cooking start time assuming no Orders are present
		...
		for (int j=0;j<60;j++){
			...
			ready_Time.add(Calendar.MINUTE,-1);
						...
						for(Orders ordz:os){
									...
									if((((fulfillment_Starttimes.after(fulfillment_Starttime))||(fulfillment_Starttimes.equals(fulfillment_Starttime)))&&(fulfillment_Starttimes.before(ready_Time)|| fulfillment_Starttimes.equals(ready_Time)))||((ready_Times.after(fulfillment_Starttime)||ready_Times.equals(fulfillment_Starttime))&&(ready_Times.before(ready_Time)||ready_Times.equals(ready_Time)))||(fulfillment_Starttimes.before(fulfillment_Starttime)&&ready_Times.after(ready_Time))){
	...
	customerOrders = (ArrayList)oservice.getCustomerOrder(Integer.parseInt(session.getAttribute("userid").toString()));
